...
export default class Main extends React.Component {
  ...
  onSearch(vals={}) {
    ...
    return getListData('dataanalysis/littletoformal', vals).then(res => {
      if(res.status){
        this.setState({
          loading: false,
          data: res.data
  ...
  render () {
    const columns = [{
      ...
      render: (val, record) => {
        if (record.ChannelName1 == '合计') {
          ...
          return (
            <span className="href" onClick={() => this.goDetail(record.ChannelId, '小规模明细表')}>{val}</span>
          )
        }
      }
    }, {
      ...
      render: (val, record) => {
        if (record.ChannelName1 == '合计') {
          ...
          return (
            <span className="href" onClick={() => this.goDetail(record.ChannelId, '小规模转一般纳税人明细表')}>{val}</span>
          )
        }
      }
    }]
    ...
    // const datasource = [{"CreateDate":"2018-02-26","PartitionName":"西北区","parcode":38,"ccode":"650100","pcode":"650000","ProvinceName":"新  疆","ChannelId":"106501161010000035","pChannelid":"106501161010000035","ChannelName1":"新疆同城定制商务咨询有限公司","ChannelName2":"","CityName":"乌鲁木齐市","Status":1,"littlenum":13.0,"formalnum":0.0}]
    let datalast
    ...
    return (
      <div className="more-notice recharge-apply" style={{ background: '#fff', padding: '10px' }}>
        <h3 className="vheader">小规模转一般纳税人统计</h3>
