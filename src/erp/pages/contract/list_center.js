...
class Main extends Component {
  ...
  discontinue = (row) => {
    ...
    const dialog = Dialog({
      content: <DiscontinueDialog close={() => { dialog.close() }} data={row} contractId={row.ContractNo} ref={v => { v && (v.handler = dialog) }} />,
      width: 1100,
      confirmLoading: false,
      footer: null,
  ...
  viewDiscontinue  = (row)=>{
    const dialog = Dialog({
        content: <DiscontinueDialog close={()=>{dialog.close(true)}} data={row} reviewable={false} ref={v => { v && (v.handler = dialog) }} />,
        width: 1100,
        confirmLoading: false,
        footer: null,
        title: row.CompanyName || '中止合同'
  ...
  render () {
    ...
    return (
      <div>
        ...
        />
        <div>
          <Button type="primary" key='pzf' onClick={this.plDiscontinue.bind(this,2)}>批量作废</Button>
        </div>
      </div>
    );
  }
