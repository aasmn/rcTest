...
import UserForm from '@/containers/users/channelUserForm'
...
class UserList extends React.Component {
  ...
  handleSearch (params) {
    ...
    channelApi.user.get(params).then(res => {
      console.log(res)
      if (res.status) {
        this.setState({
          data: res.data.list
  ...
  handleRemove (record) {
    ...
    const modal = Modal.show({
      ...
      mask: true,
      onOk: () => {
        channelApi.user.delete(record.mobile).then(res => {
          if (res.status) {
            message.info('操作成功！')
            this.handleSearch()
          }
    ...
    const modal = Modal.show({
      ...
      onOk: () => {
        ...
        this.userform.validateFields((err, values) => {
          if (!err) {
            channelApi.user[method](values).then(res => {
              if (res.status) {
                this.onSearch()
                modal.hide()
              }
